...
export default function Dashboard({ user, updateUser, onLogout }) {
  ...
  function toggleExercise(index) {
    ...
    if (completedAll) {
      ...
      updateUser(updatedUser);
      persistUser(updatedUser);
      alert('🔥 Congratulations! All exercises completed today!');
    } else {
  ...
  return (
    <div className="min-h-screen flex flex-col">
      {/* Header */}
      <div className="glass-card p-4 flex justify-between items-center mb-6 w-full relative">
        <TaskManager user={user} updateUser={updateUser} pointsPerTask={POINTS_CONFIG.taskComplete} />
        <h1 className="text-4xl font-bold absolute left-1/2 transform -translate-x-1/2">Flow-State</h1>
      ...
      {/* Profile & info */}
      <div className="flex justify-center items-center mt-6 relative space-x-6 px-6">
        <div className="glass-card px-4 py-2 text-center text-sm font-medium w-28">
        ...
        <div className="flex flex-col items-center glass-card p-4">
          ...
          <div className="text-sm text-gray-300 mt-1">
            {user.tasksCompleted || 0} tasks • Rank {rank}
          </div>
        </div>
      ...
      {/* Streak & Rank */}
      <div className="mt-6 px-6 grid gap-6 sm:grid-cols-1 md:grid-cols-2">
      ...
      <div className="mt-6 px-6 flex-1">
        ...
        {allExercises.filter(n => !exercises.some(e => e.name === n)).length > 0 && (
          <div className="mb-4 glass-card p-4">
            <label className="block mb-2 font-medium">Add Exercise:</label>
            <select
              onChange={(e) => { if (e.target.value) { addExercise(e.target.value); e.target.value = ''; } }}
              className="w-full bg-black/40 border border-cyan-500 rounded px-3 py-2 text-white"
            >
        ...
        <div className="mt-4 glass-card p-4">
          <div className="flex items-center justify-between mb-3">
            ...
            <button className="text-sm text-cyan-400 hover:underline" onClick={() => setShowFriendsBox(!showFriendsBox)}>
          ...
          {showFriendsBox && (
            <div className="grid gap-3">
              {friendsList.length === 0 && (
                <div className="p-3 rounded text-sm text-gray-400">No friends yet. Add from Leaderboard.</div>
              )}
              ...
              {friendsList.map(email => {
                ...
                if (!fd) return null;
                return (
                  <div key={email} className="glass-card p-3 flex items-center justify-between">
                    <div>
                      <div className="font-medium">{fd.name}</div>
                      <div className="text-xs text-gray-400">Rank {fd.rank} • Streak {fd.streak}d</div>
                    </div>
                    <div className="text-sm text-gray-300">#{fd.position}</div>
                  </div>
                );
              })}
            </div>
      ...
      <footer className="mt-auto relative">
        <div className="flex justify-between items-end w-full px-6 pb-6">
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="flex items-center gap-2 bg-pink-600 text-white px-4 py-2 rounded-r-2xl shadow hover:scale-105 transition"
          >
      ...
      {showRequestsModal && (
        <div className="fixed inset-0 z-70 flex items-center justify-center bg-black/60 p-4">
          <div className="glass-card w-full max-w-md overflow-auto">
            <div className="flex items-center justify-between px-4 py-3 border-b border-gray-700">
              <h3 className="text-lg font-semibold">Friend Requests</h3>
              <button onClick={() => setShowRequestsModal(false)} className="text-gray-400 hover:text-white">✖</button>
            </div>

            <div className="p-4">
              {pendingRequests.length === 0 && <div className="py-6 text-center text-gray-400">No requests</div>}

              {pendingRequests.map(req => {
                const users = JSON.parse(localStorage.getItem('users') || '[]');
                const requester = users.find(u => u.email === req);
                return (
                  <div key={req} className="flex items-center justify-between border-b border-gray-700 py-3">
                    <div>
                      <div className="font-medium">{requester?.name || req}</div>
                      <div className="text-xs text-gray-400">{req}</div>
                    </div>
                    <div className="flex gap-2">
                      <button onClick={() => acceptRequest(req)} className="btn-glow px-3 py-1">Accept</button>
                      <button onClick={() => rejectRequest(req)} className="px-3 py-1 bg-gray-700 text-white rounded hover:bg-gray-600">Reject</button>
                    </div>
                  </div>
                );
              })}
            ...
            <div className="flex justify-end gap-3 px-4 py-3 border-t border-gray-700">
              <button onClick={() => setShowRequestsModal(false)} className="px-3 py-1 rounded bg-gray-700 hover:bg-gray-600">Close</button>
            </div>
          </div>
        </div>
      )}
